extends Node2D
## The actual game: shared 2D world (camera + puzzle buttons) plus the
## in-game UI (connection status, offset-mode toggle, solve button, hud).
## Entered from MainMenu once a host/join is kicked off.
...
@onready var solve_button: Button = $UI/SolveButton
...
func _ready() -> void:
	...
	#_generate_landmarks()
	camera.make_current()
	offset_checkbox.toggled.connect(camera.set_offset_mode)
	solve_button.pressed.connect(func(): print(PuzzleSolver.solve_puzzle()))
